...
const Main = () => {
  ...
  return(
    <div>
      <div className='flex items-center justify-center flex-col h-200 bg-[#2c2b2d] p-6 shadow-2xl shadow-black bg-gray-400'>
        <Swiper breakpoints={{340: {slidesPerView: 2, spaceBetween:15},700: {slidesPerView: 3, spaceBetween: 15}}} loop={true} freeMode={true} pagination={{ clickable: true}} modules={[FreeMode, Pagination]} className='max-w-[90%] lg:max-w-[80%]96 mt-7 mb-7'>
          {ServiceData.map((item) => (
            <SwiperSlide key={item.title}>
              <Link to={`/filmek/${item.id}`}><div className='flex flex-col gap-6 group relative shadow-lg text-white rounded-xl px-6 py-8 h-[250px]  w-[215px] lg:h-[650px] lg:w-[550px]'>
